...
use crate::{
    ipc::{
        AssemblyMetadataEntry, AttributeMetadataEntry, ModuleMetadataEntry, ResourceMetadataEntry,
    },
    state::AppState,
};
...
pub fn ExplorerPanel(
    ...
    let mut collapsed_metadata_sections = use_signal(BTreeSet::<String>::new);

    let selected_id = state.selected_id.read().clone();
    let assemblies = state.assemblies.read().clone();

    let explore_result = selected_id.as_ref().and_then(|id| {
    ...
    let methods = explore_result
        .as_ref()
        .map(extract_methods)
        .unwrap_or_default();
    let grouped_methods = group_methods_by_namespace(&methods);
    let assembly_metadata = explore_result
        .as_ref()
        .map(|result| {
            result
                .explore
    ...
    let has_metadata = !assembly_metadata.assembly_name.is_empty()
        || !assembly_metadata.full_name.is_empty()
        || !assembly_metadata.modules.is_empty()
        || !assembly_metadata.assembly_references.is_empty()
        || !assembly_metadata.resources.is_empty()
        || !assembly_metadata.custom_attributes.is_empty();
    ...
    rsx! {
        div {
            ...
            MetadataSection {
                ...
                title: "Embedded resources".to_string(),
